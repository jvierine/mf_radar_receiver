...
while True:
    ...
    print(i0)
    # 
    tvec,rvec,fvec,RTI1,RDI1=crti.rti(d,"ch1",i0,n_samples=dt*1000000,plot=False)
    ...
    for c in ["ch1","ch2","ch3","ch4"]:
        tvec,rvec,fvec,RTI2,RDI2=crti.rti(d,c,i0,n_samples=dt*1000000,plot=False)
        rtis.append(RTI2)
        rdis.append(RDI2)
        if False:
            dB=10.0*n.log10(n.abs(RDI2.T)**2.0)
            nfloor=n.median(dB)
            plt.pcolormesh(fvec,rvec,dB-nfloor,cmap="turbo",vmin=-3,vmax=40)
            plt.xlabel("Doppler (Hz)")
            plt.ylabel("Ranmge (km)")
            cb=plt.colorbar()
...
#        XC=RTI1.T*n.conj(RTI3.T)
        ...
        plt.title(mc.unix2datestr(i0/1e6))
        cb=plt.colorbar()
        cb.set_label("Power (dB)")
        plt.xlabel("Time (s)")
        plt.ylim([0,400])
