...
# The script will produce one 2-day plot per 2-day window starting here,
...
# Detection thresholds (applied to reference dipole ch3 only)
NOISER0        = 50
NOISER1        = 110
NOISE_FMIN     = 5
SNR_THRESH     = 20
...
# AoA quality gate (radians). Detections with RMS phase residual above
# this on the dipole baselines are discarded.
PHASE_RESIDUAL_MAX = 0.8
...
def fit_horizontal_wind(det, min_points=8):
    ...
    det columns:
        ...
        3  height km   7  los_v     11 phase_residual rad
    """
...
def extract_detections_for_interval(dmt, phasecal, start_unix, end_unix):
    """
    Channel mapping:
        RDI1 → ch1 dipole  (phase cal index 0)
        RDI2 → ch2 loop    (phase cal index 1)  — disambiguation only
        RDI3 → ch3 dipole  (phase cal index 2)  — reference / SNR
        RDI4 → ch4 dipole  (phase cal index 3)

    Cross-correlations:
        xc13 = RDI1 × RDI3*  ch1-ch3  dipole fine AoA
        xc14 = RDI1 × RDI4*  ch1-ch4  dipole fine AoA
        xc34 = RDI3 × RDI4*  ch3-ch4  dipole fine AoA
        xc32 = RDI3 × RDI2*  ch3-ch2  loop disambiguation (60.6 m, 0.56λ)

    Returns array, columns:
        0  time unix   4  vr m/s    8  los_w        11 phase_residual rad
        1  x km        5  snr       9  range km
        2  y km        6  los_u     10 doppler Hz
        3  height km   7  los_v
    """
    ...
    for read_start in np.arange(t0_us, t1_us, int(READ_DT * 1e6)):
        ...
        for k in dd.keys():
            ...
            # Noise / SNR from ch3 (reference dipole)
            try:
            ...
            noise_pwr = 0.5 * (
                np.mean(np.abs(RDI3[0:fi0,  ri0:ri1]) ** 2.0)
                + np.mean(np.abs(RDI3[fi1:-1, ri0:ri1]) ** 2.0)
            )
            if noise_pwr <= 0 or not np.isfinite(noise_pwr):
                continue

            snr      = (np.abs(RDI3) ** 2.0 - noise_pwr) / noise_pwr
            rr, ff   = np.meshgrid(rvec, fvec)

            # Background rejection: discard range gates where a large fraction
            # of Doppler bins exceed SNR_THRESH (broad clutter, not meteors)
            ...
            # Dipole cross-correlations — full SNR, used for fine AoA
            xc13 = (RDI1[idx] * np.conj(RDI3[idx])).flatten()
            xc14 = (RDI1[idx] * np.conj(RDI4[idx])).flatten()
            xc34 = (RDI3[idx] * np.conj(RDI4[idx])).flatten()
            # Loop cross-correlation — disambiguation only
            xc32 = (RDI3[idx] * np.conj(RDI2[idx])).flatten()
            ...
            for mi in range(len(ffs)):
                los_u, los_v, los_w, ph_res = ih.aoa(
                    xc_dipole=[xc13[mi], xc14[mi], xc34[mi]],
                    xc_loop=xc32[mi],
                    wavelength=mc.wavelength,
                )

                if ph_res > PHASE_RESIDUAL_MAX:
                    continue
...
def plot_window(wind_rows, window_start_unix, window_end_unix, plot_file):
    """
    Plot eastward and northward wind for one 2-day window.
    wind_rows already filtered to [window_start_unix, window_end_unix).
    """
    if wind_rows is None or wind_rows.shape[0] == 0:
        print(f"  No wind data for this window, skipping plot.")
        return
...
def main():
    ...
    # Determine the last timestamp available in the data
    bounds              = dmt.get_bounds()
    newest_available    = bounds[1] / 1e6   # unix seconds

    # First window starts at PROCESS_START (UTC midnight)
    ...
    while True:
        ...
        # Only process windows that are fully covered by available data
        ...
        while next_block < window_end:
            ...
            det_block = extract_detections_for_interval(
                dmt, phasecal, block_start, block_end
            )
            ...
            if det_block.shape[0] > 0:
                ...
                # Insert empty rows so the time axis stays complete
                h_edges   = np.arange(HEIGHT_MIN, HEIGHT_MAX + HEIGHT_RES, HEIGHT_RES)
                h_centers = 0.5 * (h_edges[:-1] + h_edges[1:])
            ...
            # Accumulate detections and winds for this window
        ...
        # Filter strictly to [window_start, window_end) before plotting
